...
export const searchPlaylists = async (
  ...
  const data: { playlists: { items: SpotifyPlaylist[] } } = await spotifyFetch(
...
export const fetchPlaylistTracks = async (
  ...
  let url:
    | string
    | null = `/playlists/${playlistId}/tracks?limit=100&fields=items(track(id,uri,name,artists(id,name))),next`;
  ...
  while (url && tracks.length < maxTracks) {
    ...
    data.items.forEach((item: { track: SpotifyTrack | null }) => {
      if (item.track) {
...
export const fetchArtistAlbums = async (
  ...
  let url:
    | string
    | null = `/artists/${artistId}/albums?include_groups=album,single&limit=50&market=${market}`;
  while (url) {
    const data: { items: { id: string }[]; next: string | null } =
      await spotifyFetch(url, token);
...
export const fetchAlbumTracks = async (
  ...
  const tracks: SpotifyTrack[] = [];
  while (url) {
    const data: { items: SpotifyTrack[]; next: string | null } =
      await spotifyFetch(url, token);
